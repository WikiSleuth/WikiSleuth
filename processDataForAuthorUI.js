function buildAuthorHTMLToAdd(tabs, data, callback) {
		if (data.length > 0) {
			html = "<div id='panel'> <h2>" + data[0]['user'] + " Revision History<span class= 'shortcut'>Press CMD+Shift+F to search for new author</span></h2><h4> Total Edits: </h4>" + 
					"<h3> Recent Revision List</h3><br>";
			for (i=0; i<data.length; i++) {
				html += "<li>" +
						   "<span class='timestamp'>"+ "Revision Date: " + data[i]['timestamp']+"</span> <span class='editor'>"+ "Page id: " + data[i]['title']+"</span> <span class='comment'>"+"Reason For Edit" + data[i]['comment']+"</span>" +
